...
import { useCreateTraderRefBluk } from '@/services/master/tradersref/services';
...
export const TraderReferenceBulkUpload = () => {
  ...
  const csvFields: any = import.meta.env.VITE_CUSTOMERS_TRADER_REFERENCE_BULK_UPLOAD_CSV_FIELDS ? JSON.parse(import.meta.env.VITE_CUSTOMERS_TRADER_REFERENCE_BULK_UPLOAD_CSV_FIELDS) : [];
  ...
  const customerForm = useForm({
    onValidSubmit: () => {
      ...
      uploadTraderReferences.mutate(payloadItems as any);
    },
  });

  const uploadTraderReferences = useCreateTraderRefBluk({
    onSuccess: ({ created_references, errors }) => {
      toastSuccess({
        title: 'Uploaded finished successfully',
        description: `No of Trader uploaded: ${uploadedRows.length} Successful: ${created_references?.length} Failed: ${errors?.length}`,
      });
      let payloadVariables = JSON.parse(JSON.stringify(uploadedRows));
      if (created_references && created_references.length > 0) {
        const updatedItems = payloadVariables.filter(
          (obj1: any) =>
            !created_references.some(
      ...
      if (errors && errors.length > 0) {
        ...
        const updatedArray1 = payloadVariables.map((obj1: any) => {
          const matchingObj: any = errors.find(
            (obj2: any) => obj2.customer_id === obj1.customer_id
          );
      ...
      <Stack pl={2} spacing={4}>
        <HStack justify={'space-between'}>
          ...
          <Button
            ...
            onClick={() =>
              handleDownload(
                import.meta.env.VITE_CUSTOMERS_TRADER_REFERENCES_CSV
              )
            }
          >
            Download Sample
          ...
          <Formiz autoForm connect={customerForm}>
                  ...
                  {uploadedRows &&
                    ...
                    uploadedRows.map((item: any, index: number) => (
                      ...
                      >
                        ...
                        <Td>
                          <FieldInput
                            key={`vendor_name_${fileKey}_${index + 1}`}
                            name={`vendor_name_${index + 1}`}
                            ...
                            maxWidth="100%"
                            onValueChange={(value) =>
                              handleInputChange(value, 'vendor_name', index)
                            }
                          />
                        </Td>
                        <Td>
            ...
            >
              <Button
                type="button"
                colorScheme="red"
                disabled={uploadTraderReferences.isLoading}
              ...
              <Button
                type="submit"
                colorScheme="brand"
                isLoading={uploadTraderReferences.isLoading}
                isDisabled={
                  uploadedRows.length === 0 || uploadTraderReferences.isLoading
                }
              >
                Submit
              </Button>
...
export default TraderReferenceBulkUpload;
