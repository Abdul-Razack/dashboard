...
import { TraderRefDataColumn } from '@/services/master/tradersref/schema';
import { useTraderRefIndex } from '@/services/master/tradersref/services';

const TraderReferenceMaster = () => {
  ...
  const { data: listData, isLoading: listLoading } =
    useTraderRefIndex(queryParams);
  const data = listData?.data ?? [];

  const columnHelper = createColumnHelper<TraderRefDataColumn>();
        ...
        return (
          <HStack spacing={4} justify={'flex-end'}>
            <IconButton
              ...
              size={'sm'}
              onClick={() =>
                navigate(`/trader-reference-master/${info.row.original.id}`)
              }
            />
            <IconButton
              aria-label="Edit"
              ...
              onClick={() =>
                navigate(
                  `/trader-reference-master/${info.row.original.id}/edit`
                )
              }
            />
      ...
      <Stack pl={2} spacing={4}>
        <HStack justify={'space-between'}>
          ...
          <ResponsiveIconButton
            ...
            icon={<LuPlus />}
            size={{ base: 'sm', md: 'md' }}
            onClick={() => navigate('/trader-reference-master/create')}
          >
            Add New trader reference
          </ResponsiveIconButton>
        </HStack>
...
export default TraderReferenceMaster;
