...
type TraderReferenceProps = {
  traderReferenceData: any;
  refreshFunction: () => void;
  customerId: any;
};

export function TraderReference({
  traderReferenceData,
  refreshFunction,
  customerId,
}: TraderReferenceProps) {
  const {
    isOpen: isModalOpen,
    onOpen: onModalOpen,
    ...
    >
      ...
      >
        <Table variant="striped" size={'sm'}>
          ...
          <Tbody>
            {traderReferenceData &&
              traderReferenceData.map((item: any, index: number) => (
                <Tr key={index}>
                  <Td>{index + 1}</Td>
                  <Td> {item?.attention ?? '-'}</Td>
                  <Td>
                    <Text>{item?.address ?? '-'} <br/> {item?.address_line2 ?? '-'}</Text>
                  </Td>
                  <Td> {item?.city ?? '-'}</Td>
                  <Td> {item?.country ?? '-'}</Td>
                  <Td> {item?.email ?? '-'}</Td>
              ...
              {traderReferenceData && traderReferenceData.length === 0 && (
              <Tr>
