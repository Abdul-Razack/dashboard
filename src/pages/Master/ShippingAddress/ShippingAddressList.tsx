...
import { ShippingDataColumn } from '@/services/master/shipping/schema';
import { useShippingAddressIndex } from '@/services/master/shipping/services';

const ShippingAddressMaster = () => {
  ...
  const { data: listData, isLoading: listLoading } =
    useShippingAddressIndex(queryParams);
  const data = listData?.data ?? [];

  const columnHelper = createColumnHelper<ShippingDataColumn>();
        ...
        return (
          <HStack spacing={4} justify={'flex-end'}>
            <IconButton
              ...
              size={'sm'}
              onClick={() =>
                navigate(`/shipping-address-master/${info.row.original.id}`)
              }
            />
            <IconButton
              aria-label="Edit"
              ...
              onClick={() =>
                navigate(
                  `/shipping-address-master/${info.row.original.id}/edit`
                )
              }
            />
      ...
      <Stack pl={2} spacing={4}>
        <HStack justify={'space-between'}>
          ...
          <ResponsiveIconButton
            ...
            icon={<LuPlus />}
            size={{ base: 'sm', md: 'md' }}
            onClick={() => navigate('/shipping-address-master/create')}
          >
                  ...
                  <>
                    ...
                    <AccordionPanel p={0}>
                      ...
                      >
                        ...
                        />
                        <FieldInput
                          key={`consignee_name_${formKey}`}
                          name="consignee_name"
                          placeholder="Enter Consignee Name"
                          w={{ base: 'full', md: '20%' }}
                          size={'sm'}
                          onValueChange={(value) =>
                            handleInputChange(value, 'consignee_name', 'four')
                          }
                        />
                        <Button
                          type="reset"
...
export default ShippingAddressMaster;
