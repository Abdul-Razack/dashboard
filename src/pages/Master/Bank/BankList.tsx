...
import { bankdataColumn } from '@/services/master/bank/schema';
import { useBankIndex } from '@/services/master/bank/services';
import {
  useCustomerList,
  useCustomerListCode,
...
const BankList = () => {
  ...
  const {data: listData, isLoading: listLoading} = useBankIndex(queryParams);
  const data = listData?.data ?? [];

  const columnHelper = createColumnHelper<bankdataColumn>();
  ...
  const columns = [
    ...(types === 'one'
      ? [
         columnHelper.accessor('id', {
          ...
          columnHelper.accessor('bank_ac_iban_no', {
            cell: (info) => info.getValue(),
            header: () => 'IBAN No.',
          }),
    ...
    ...(types === 'four'
      ...
      : []),
    columnHelper.accessor('bank_swift', {
      cell: (info) => info.getValue(),
      header: 'Swift Code',
    }),
    ...
    columnHelper.accessor('type_of_ac', {
      cell: (info) => info.getValue(),
      header: () => 'Type of AC',
    }),
    ...
    columnHelper.accessor('bank_email', {
      cell: (info) => info.getValue(),
      header: 'Email',
    }),
        ...
        return (
          <HStack spacing={4} justify={'flex-end'}>
            <IconButton
              ...
              icon={<ViewIcon />}
              size={'sm'}
              onClick={() => navigate(`/bank-master/${info.row.original.id}`)}
            />
            <IconButton
              aria-label="Edit"
              icon={<EditIcon />}
              size={'sm'}
              onClick={() =>
                navigate(`/bank-master/${info.row.original.id}/edit`)
              }
            />
      ...
      <Stack pl={2} spacing={4}>
        <HStack justify={'space-between'}>
          ...
          <ResponsiveIconButton
            ...
            icon={<LuPlus />}
            size={{ base: 'sm', md: 'md' }}
            onClick={() => navigate('/bank-master/create')}
          >
            Add New bank
          </ResponsiveIconButton>
        </HStack>
                  ...
                  <>
                    ...
                    <AccordionPanel p={0}>
                      ...
                      >
                        ...
                        />
                        <FieldInput
                          key={`bank_ac_iban_no_${formKey}`}
                          name="bank_ac_iban_no"
                          placeholder="Bank IBAN No"
                          w={{ base: 'full', md: '20%' }}
                          size={'sm'}
                          onValueChange={(value) =>
                            handleInputChange(value, 'bank_ac_iban_no', 'four')
                          }
                        />
                        <Button
                          type="reset"
            ...
            <Box width="100%" bg={'green.200'} p={4} borderRadius={4}>
              ...
              >
                ...
                <FieldInput
                  ...
                  size={'sm'}
                  onValueChange={(value) =>
                    handleInputChange(value, 'beneficiary_name', 'three')
                  }
                />
                <FieldInput
                  key={`bank_ac_iban_no_${formKey}`}
