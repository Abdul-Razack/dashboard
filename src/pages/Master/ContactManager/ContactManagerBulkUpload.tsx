...
import { useCreateContactManagerBluk } from '@/services/master/contactmanager/services';
...
export const ContactManagerBulkUpload = () => {
  const navigate = useNavigate();
  const toastSuccess = useToastSuccess();
  const toastError = useToastError();

  const csvFields: any = import.meta.env
    .VITE_CUSTOMER_CONTACT_MANAGEMENT_BULK_UPLOAD_CSV_FIELDS
    ? JSON.parse(
        import.meta.env.VITE_CUSTOMER_CONTACT_MANAGEMENT_BULK_UPLOAD_CSV_FIELDS
      )
    : [];
  ...
  const customerForm = useForm({
    onValidSubmit: () => {
      ...
      uploadContactManager.mutate(payloadItems as any);
    },
  });
  ...
  const uploadContactManager = useCreateContactManagerBluk({
    onSuccess: ({ created_contacts, errors }) => {
      toastSuccess({
        title: 'Uploaded finished successfully',
        description: `No of contacts uploaded: ${uploadedRows.length} Successful: ${created_contacts?.length} Failed: ${errors?.length}`,
      });
      let payloadVariables = JSON.parse(JSON.stringify(uploadedRows));
      if (created_contacts && created_contacts.length > 0) {
        const updatedItems = payloadVariables.filter(
          (obj1: any) =>
            !created_contacts.some(
      ...
      <Stack pl={2} spacing={4}>
        <HStack justify={'space-between'}>
          ...
          <Button
            ...
            onClick={() =>
              handleDownload(
                import.meta.env.VITE_CUSTOMERS_CONTACT_MANAGERS_CSV
              )
            }
          >
            Download Sample
          ...
          <Formiz autoForm connect={customerForm}>
              ...
              <Table variant="simple" size="sm">
                <Thead bg="gray.500">
                  <Tr>
                    ...
                    <Th color="white">Address Line2</Th>
            ...
            >
              <Button
                type="button"
                colorScheme="red"
                disabled={uploadContactManager.isLoading}
              ...
              <Button
                type="submit"
                colorScheme="brand"
                isLoading={uploadContactManager.isLoading}
                isDisabled={
                  uploadedRows.length === 0 || uploadContactManager.isLoading
                }
              >
                Submit
              </Button>
...
export default ContactManagerBulkUpload;
