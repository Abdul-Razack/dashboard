...
const SupplierQuotationCreate = () => {
  ...
  const [rfqItemId, setRfqItemId] = useState<number | null>(null);
  ...
  const [isNoQuote, setIsNoQuote] = useState<boolean>(false);
  const [hasNoQuotation, setHasNoQuotation] = useState<boolean>(false);
  const [showNQButton, toggleNQButton] = useState<boolean>(false);
  ...
  const getQuotationItems = async (partNumberID: Number) => {
    try {
      ...
      setHasNoQuotation(respData?.is_no_quotation);
      toggleNQButton(respData?.is_show_no_quotation);
  ...
  useEffect(() => {
    if (quotationId && rfqItemId && rfqId) {
      if (rfqItems[activeTab]) {
        setLoading(true);
        getQuotationItems(rfqItems[activeTab].part_number_id);
      }
    }
  }, [quotationId, rfqItemId, rfqId]);
  ...
  useEffect(() => {
    if (customerDetails?.data) {
      ...
      form.setValues({
        currency_id: customerDetails?.data?.currency_id.toString(),
      });
    }
  }, [customerDetails]);
  ...
  useEffect(() => {
    if (rfqDetails?.data.items) {
      console.log(rfqDetails?.data.items)
  ...
  const markQuotationItemAsNQ = useMarkQuotationAsNQ({
    onSuccess: (data) => {
      toastSuccess({
        title: `Mark as No Quote`,
  ...
  useEffect(() => {
    if (rfqItems[activeTab]) {
      setRfqItemId(rfqItems[activeTab].id);
    }
    setTimeout(() => {
      setLoading(false);
    }, 3000);
  ...
  const handleConfirm = () => {
    if (isDuplicate) {
      ...
      markQuotationItemAsNQ.mutate({
        quotation_id: quotationId ?? 0,
        rfq_item_id: rfqDetails?.data.items[activeTab].id ?? 0,
      });
      //createQuoteItem.mutate(quoteItemPayload);
      handleClose();
    } else {
  ...
  const triggerNoQuote = () => {
    setIsNoQuote(true);
    setConfirmationTitle('Set as No-Quote!!');
    setConfirmationContent(
      'Are you sure want to make this item to set as No-Quote?'
    );
    setIsOpen(true); // Close the modal on cancel or outside click
  };
              ...
              >
                <FieldSelect
                  ...
                  onValueChange={(value) => {
                    ...
                    setCustomerDetails({});
                    setLoading(true);
                    setHasNoQuotation(false);
                    toggleNQButton(false);
                    setQuotationId(null);
                    setRfqItemId(null);
                  }}
                    ...
                    <TabPanel p={4} key={`panel-${index}`}>
                      ...
                      <Formiz connect={itemForm}>
                        <form noValidate onSubmit={itemForm.submit}>
                          {rfqDetails?.data.items[activeTab].is_no_quotation !==
                            true && (
                            <Stack
                              bg="white"
                              borderRadius="md"
                            ...
                            >
                              <Box flex="2.5">
                                <FieldSelect
                                  ...
                                  isDisabled={quotationId == null || hasNoQuotation}
                                />
                              </Box>
                              <Box flex="1.5">
                                <FieldSelect
                              ...
                              <Box flex="1.5">
                                <FieldSelect
                                  ...
                                  isDisabled={uomDisabled || hasNoQuotation}
                            ...
                            hasNoQuotation === false && (
                              ...
                              >
                                <Button
                                  ...
                                  isLoading={markQuotationItemAsNQ.isLoading}
                                  isDisabled={markQuotationItemAsNQ.isLoading}
                                >
                                  Set as No Quote
                                </Button>
                              </Stack>
...
export default SupplierQuotationCreate;
