...
export const SELCreate = () => {
  ...
  const [newCustomerName, setCustomerName] = useState<string>('');
  const {
    isOpen: isNewContactModalOpen,
    onOpen: onNewContactModalOpen,
    onClose: onNewContactModalClose,
  } = useDisclosure();
  ...
  const fetchContactInfo = fetchContactManagerInfo();
  const fetchShippingAddress = fetchShippingAddressInfo();
  ...
  const handleCloseCustomerModal = (status?: boolean, id?: any) => {
    setCustomerSelectKey((prevKey) => prevKey + 1);
    if (status) {
  ...
  const handleCloseShippingAddressModal = (status: boolean, id: any) => {
    ...
    if (status) {
      ...
      if (id) {
        ...
        setTimeout(() => {
          form.setValues({
            [`customer_shipping_address_id`]: id.toString(),
          });
        }, 2000);
      }
    }
  ...
  const customerListCode: UseQueryResult<QueryData, unknown> =
    useCustomerListCode();
  ...
  const handleOpenModal = (item_id: number) => {
    ...
    popupVariables.receiptOptions = receiptOptions;
  ...
  const handleSpareCreate = (inputValue: string, fieldName: string) => {
    openSpareCreateModal(fieldName, inputValue);
  };

  const handleAddNewCustomer = (inputValue: string) => {
    setCustomerName(inputValue);
    onNewContactModalOpen();
  };

  const openSpareCreateModal = (fieldName: string, inputValue?: string) => {
    setSelectedPartNum(fieldName);
    setSpareName(inputValue ?? '');
    onNewSpareModalOpen();
  };
      ...
      if (duplicateCount === 0) {
        const payload = {
          ...
          customer_shipping_address_id: Number(
            values?.customer_shipping_address_id
          ),
  ...
  const getCustomerInfo = async (customerId: number) => {
    try {
      ...
      const addresses = response?.data?.customer_shipping_addresses || [];
      ...
      setContactId(response?.data?.currency_id.toString() || '');
      setPaymentModeId(response?.data?.payment_mode_id.toString() || '');
      setPaymentTermId(response?.data?.payment_term_id.toString() || '');
      const firstManager = response?.data?.customer_contact_managers?.[0];
      if (firstManager?.id) {
        setContactManager(Number(firstManager.id));
      } else {
        setContactManager(0);
      }
      if (options.length > 0) {
        const firstAddressId = Number(options[0].value);
        // form.setValues({[`customer_shipping_address_id`]: firstAddressId});
        setShippingAddress(firstAddressId);
      } else {
        // form.setValues({ [`customer_shipping_address_id`]: '' });
        setShippingAddress(0);
        setFullVendorAddress('N/A');
      }
    } catch (err) {
  ...
  useEffect(() => {
    const fetchAndFormatShippingAddress = async () => {
      if (shippingAddress) {
        try {
          const shippingData = await fetchShippingAddress(shippingAddress);
          if (shippingData) {
            setFullVendorAddress(formatShippingAddress(shippingData));
          } else {
    ...
    fetchAndFormatShippingAddress();
  }, [shippingAddress]);

  useEffect(() => {
    const fetchAndFormatContact = async () => {
      if (contactManager) {
        try {
          const contactData = await fetchContactInfo(contactManager);
          if (contactData) {
            setFullContactAddress(formatContactAddress(contactData));
          } else {
          ...
          <Stack spacing={0}>
            ...
            >
              ...
              <BreadcrumbItem isCurrentPage color={'gray.500'}>
                <BreadcrumbLink>Create SEL</BreadcrumbLink>
              </BreadcrumbItem>
            </Breadcrumb>
        ...
        >
          ...
          <Formiz autoForm connect={form}>
            ...
            <Stack spacing={2}>
              ...
              >
                <FieldSelect
                  key={`customer_${customerSelectKey}`}
                  label={'Customer Name'}
                  name={'customer_id'}
                  ...
                  selectProps={{
                    ...
                    onCreateOption: (inputValue) =>
                      handleAddNewCustomer(inputValue),
                  }}
                  onValueChange={(value) => {
                    if (value === 'add_new') {
                      onNewContactModalOpen();
                    } else {
                      selectedCustomer.current = Number(value) ?? 0;
                ...
                <FieldDisplay
                  label="Customer Code"
                  value={
                    customerCodeOptions?.find(
                      (item: any) => Number(item.value) === customer_id
                    )?.label || 'N/A'
                  }
                  size={'sm'}
                ...
                />
                <FieldSelect
                  key={`shipping_contact_${shippingAddress}_${shippingOptions.length}_${resetKey}`}
                ...
                />
                <FieldDisplay
                  key={`shipping_address_${shippingAddress}`}
              ...
              >
                ...
                />
                <FieldSelect
                  key={`payment_mode_id_${paymentModeId}`}
                  label={'Payment Mode'}
                  name={'payment_mode_id'}
                  required={'payment mode required'}
                  placeholder="Select payment mode"
                  options={paymentModeOptions}
                  defaultValue={paymentModeId}
                  size={'sm'}
                />
                <FieldSelect
                  key={`payment_terms_id_${paymentTermId}`}
                  label={'Payment Terms'}
                  name={'payment_terms_id'}
                  required={'payment terms required'}
                  placeholder="Select payment terms"
                  options={paymentTermsOptions}
                  defaultValue={paymentTermId}
                  size={'sm'}
                />
              </Stack>
              <HStack justify={'space-between'} mt={3}>
                ...
                <HStack ml="auto">
                  <Button
                    ...
                    onClick={() =>
                      handleDownload(
                        import.meta.env.VITE_SEL_SAMPLE_PARTNUMBERS_CSV
                      )
                    }
                  >
                    Download Sample
          ...
          <ContactManagerCreateModal
            ...
            customer_id={selectedCustomer.current.toString() ?? ''}
          />
