...
const PreviewPopup = ({ isOpen, onClose, data = {} }: ModalPopupProps) => {
  ...
  const [existingPRFQ, setExistingPRFQ] = useState<number>(0);
  ...
  useEffect(() => {
    if (isOpen) {
      if (data.existingPRFQ) {
        setExistingPRFQ(Number(data.existingPRFQ));
        setSelectedPRFQ(Number(data.existingPRFQ));
      }
    }
  }, [isOpen, data]);
