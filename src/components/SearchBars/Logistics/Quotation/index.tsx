...
import {
  DataColumn,
  IndexPayload,
} from '@/services/apiService/Schema/LogisticsQuotationSchema';
...
export const QuotionSearch: React.FC<SearchBarProps> = ({
  onSearchFinished,
  setModuleLoading,
  additionalParams,
  existingPRFQ,
  ...
  const QuotationList: UseQueryResult<QueryData, unknown> =
    useLogisticQuotationList();
  ...
  useEffect(() => {
    ...
    if (hasQueryParamsChanged) {
      ...
      if (queryParams?.logistic_quotation_id) {
  ...
  const getListItems = async () => {
    try {
      const data = await getAPICall(
        endPoints.index.logistic_quotation,
        IndexPayload,
        queryParams
      );
      console.log(data);
    ...
    ...(columnOrder === 'logQuoId'
      ? [
          ...
          columnHelper.accessor('lrfq', {
            cell: (info) => {
              const lrfqInfo = info.getValue();
              return format(new Date(lrfqInfo.due_date), 'dd/MM/yy') || 0;
            },
            id: 'dueDate',
            meta: {
              sortable: true,
          ...
          columnHelper.accessor('logistic_request', {
            cell: (info) => {
              const logisticRequest = info.getValue();
              return (logisticRequest?.volumetric_weight || 0.0) + ' KG';
            },
            header: 'Volum Weight',
            meta: {
              sortable: true,
          ...
          columnHelper.accessor('logistic_request', {
            cell: (info) => {
              const logisticRequest = info.getValue();
              const items = logisticRequest?.purchase_orders ?? [];
              const uniqueItems = Array.from(
                new Set(items.map((item) => item?.logistic_request_id))
              );
    ...
    ...(columnOrder === 'logisticsVendor'
      ? [
          ...
          columnHelper.accessor('customer', {
            cell: (info) => {
              const customerInfo = info.getValue();
              return customerInfo?.business_name || 'N/A';
            },
          ...
          >
            ...
            >
              ...
              >
                <FieldSelect
                  label={'Quotation No'}
                  name={'logistic_quotation_id'}
                  key={`logistic_quotation_id_${resetKey}`}
                ...
                <FieldSelect
                  ...
                  selectProps={{
                    noOptionsMessage: () => 'No LRFQ found',
                    styles: {
                      menuPortal: (base) => ({
                        ...base,
                        zIndex: 9999,
                  ...
                  isClearable={true}
                  size="sm"
                  isDisabled={queryParams?.logistic_quotation_id}
                />
              ...
              >
                ...
                <FieldDateRangePicker
                  ...
                  dateRangePickerProps={{
                    inputProps: {
                      isDisabled:
                        queryParams?.logistic_quotation_id !== '' ||
                        (queryParams?.ref_id !== '' &&
