...
export const useContactTypeIndex = (
  ...
  useQuery({
    queryKey: 'contactTypeIndex',
    queryFn: () => fetchData(endPoints.index.contact_type, zIndexPayload),
    //staleTime: 5 * 60 * 1000, // 5 minutes
    //cacheTime: 15 * 60 * 1000, // 15 minutes
    retry: 2,
    refetchOnWindowFocus: false,
...
export const useCreateContactType = (
  ...
  return useSubMasterMutation(endPoints.create.contact_type, zCreatePayload().parse, {
    ...config,
    onSuccess: (data, ...args) => {
      if (data.status) {
